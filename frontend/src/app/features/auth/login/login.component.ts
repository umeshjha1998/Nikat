...
@Component({
      ...
      <main class="auth-main">
        <header class="main-head">
          ...
          <div class="head-actions" style="display: flex; align-items: center; gap: 1rem;">
            ...
            <p style="margin: 0;">New here? <a routerLink="/register" class="link-prime">Create Account</a></p>
          </div>
        </header>
  ...
  styles: [`
    ...
    /* Visual Side */
    .auth-visual {
      flex: 1; position: relative; background: var(--card-bg); display: flex; align-items: center; padding: 5rem;
      border-right: 1px solid var(--border-color);
    }
    .v-content { position: relative; z-index: 10; max-width: 480px; }
    .v-badge {
      display: inline-block; padding: 0.5rem 1rem; border-radius: 2rem; background: var(--accent-glow);
    ...
    .v-feat .material-icons { color: var(--accent); }

    .v-blur-orb {
      position: absolute; width: 600px; height: 600px; background: radial-gradient(circle, var(--accent-glow), transparent 70%);
    ...
    .link-prime { color: var(--accent); text-decoration: none; font-weight: 700; margin-left: 0.5rem; }
    ...
    input {
      width: 100%; padding: 1.125rem 1.25rem 1.125rem 3.5rem; border-radius: 1rem; border: 1px solid var(--border-color);
      background: var(--card-bg); color: var(--text-main); font-family: inherit; font-size: 1rem; transition: all 0.2s;
    }
    input:focus { border-color: var(--accent); background: var(--glass); outline: none; box-shadow: 0 0 0 4px var(--accent-glow); }
    input.error { border-color: #f87171; }

    /* Actions */
    .actions-footer { margin-top: 2rem; }
    .btn-prime-glow {
      background: linear-gradient(135deg, var(--primary), var(--accent)); color: #fff; border: none; padding: 1.1rem; border-radius: 1.25rem;
    ...
    .btn-prime-glow:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 10px 30px var(--accent-glow); }
    ...
    .social-separator::before, .social-separator::after { content: ''; flex: 1; height: 1px; background: var(--border-color); }
    ...
    .btn-social {
      ...
      background: var(--card-bg); border: 1px solid var(--border-color); border-radius: 1rem; color: var(--text-main);
      font-weight: 700; font-size: 0.9rem; cursor: pointer; transition: 0.2s;
    }
    .btn-social:hover { background: var(--glass); border-color: var(--accent); }
